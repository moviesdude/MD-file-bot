...
async def save_batch_media_in_channel(bot: Client, editable: Message, message_ids: list):
    try:
        ...
        share_link = f"https://t.me/{Config.BOT_USERNAME}?start=MD_{str_to_b64(str(SaveMessage.id))}"
        await editable.edit(
        ...
        # ✧ source code upgraded by The sir MD_Developer 
        # ✧ Don't remove credit ✧ @MD_Bots ✧
        if(Config.LAZY_MODE == True):
            ...
            # ✧ Here is the condition for sending POST in movie channel ✧ MoviesDude_LinkZz ✧
...
async def save_media_in_channel(bot: Client, editable: Message, message: Message):
    try:
        ...
        await forwarded_msg.reply_text(
            ...
            disable_web_page_preview=True)
        share_link = f"https://t.me/{Config.BOT_USERNAME}?start=MD_{str_to_b64(file_er_id)}"
        await editable.edit(
        ...
        # ✧ source code upgraded by The sir MD Developer 
        # ✧ Don't remove credit ✧ @MD_Bots ✧
        if(Config.LAZY_MODE == True):
            thumbs= message.video.thumbs[0]
            file_id= thumbs.file_id
